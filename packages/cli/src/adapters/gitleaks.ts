...
/**
 * Translate the project's ignore globs into gitleaks `[allowlist].paths`
 * regex strings. Returns null when there are no patterns to allowlist.
 */
function buildGitleaksConfig(ignore: string[]): string | null {
  const paths = ignore
    .map(globToRegex)
    .filter(Boolean) as string[];
  if (paths.length === 0) return null;
  const escaped = paths.map((p) => `"${p.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`).join(', ');
  return `# Auto-generated by owasp-wtf to suppress scans of build output\ntitle = "owasp-wtf gitleaks allowlist"\n\n[extend]\nuseDefault = true\n\n[allowlist]\npaths = [${escaped}]\n`;
}

function globToRegex(glob: string): string | null {
  if (!glob) return null;
  // Strip leading "./"
  let g = glob.replace(/^\.\//, '');
  // Convert glob to regex covering both directory and file matches.
  const re = g
    .replace(/[.+^${}()|[\]\\]/g, '\\$&')
    .replace(/\*\*/g, '{{GS}}')
    .replace(/\*/g, '[^/]*')
...
export const gitleaksAdapter: ScannerAdapter = {
  ...
  async run(project: ProjectContext): Promise<AdapterRunResult> {
    ...
    try {
      const args = ['detect', '--no-banner', '--no-git', '--source', project.root, '--report-format', 'json', '--report-path', reportPath, '--exit-code', '0'];

      const configBody = buildGitleaksConfig(project.ignore);
      if (configBody) {
        const configPath = join(tmp, 'gitleaks.toml');
        writeFileSync(configPath, configBody, 'utf-8');
        args.push('--config', configPath);
      }

      const r = await exec('gitleaks', args, { timeoutMs: 5 * 60_000 });
